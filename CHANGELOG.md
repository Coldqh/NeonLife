...
## 0.10.0 — CITY SITUATIONS / DECISION PAUSE

### Добавлено

- единое состояние `GameSession.situations`;
- блокирующая пауза, когда миру требуется решение игрока;
- проверка активного курьерского груза патрулём;
- три варианта поведения при проверке: сотрудничество, звонок клиенту и отказ от груза;
- спор с клиентом после опоздания или повреждения посылки;
- компенсация, объяснение или перенос ответственности на диспетчера;
- ситуация перегруженной смены при низком фактическом персонале бизнеса;
- прямой личный разговор с NPC при высокой тяжести его проблемы;
- четырёхчасовой cooldown фоновых ситуаций;
- история последних сорока решений;
- отдельный адаптивный экран решения под iPhone 14 Pro;
- диагностический статус pending-ситуации и количество принятых решений;
- миграция сохранений на схему 8.

### Изменено

- важные курьерские проверки больше не закрываются автоматической строкой журнала;
- плохая доставка теперь может продолжиться человеческим конфликтом;
- низкий персонал бизнеса создаёт игровую ситуацию, а не только ухудшает число в таблице;
- тяжёлая проблема NPC может выйти в прямой разговор при физической встрече;
- продвижение времени и основные действия блокируются до принятия решения;
- выбранный ответ меняет уже существующие системы вместо отдельной шкалы выбора;
- версия приложения, PWA-кэша и серверного манифеста повышена до `0.10.0`.

### Теперь игрок может

- сохранить груз во время проверки или отказаться от него;
- использовать доверие клиента для подтверждения документов;
- компенсировать плохую доставку собственными деньгами;
- помочь перегруженному бизнесу и реально восстановить персонал;
- потребовать повышенную оплату за срочную подмену;
- выслушать человека, помочь ему или уйти;
- увидеть, как решение осталось в памяти NPC и истории мира.
