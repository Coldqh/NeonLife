...
const VENUE_PREFIXES: Record<VenueCategory, readonly string[]> = {
  ...
  gym: ["IRON", "PULSE", "BLOCK", "FORGE", "CIVIC"],
  "boxing-gym": ["RED CORNER", "SOUTHPAW", "GRIT", "TEN COUNT", "RING"],
  "shooting-range": ["VECTOR", "SIGHTLINE", "REDLINE", "CALIBER", "CIVIC"],
  "weapon-shop": ["UNDERLINE", "CUTWIRE", "REDLINE", "IRON", "BLACKMARK"]
};
...
const VENUE_SUFFIXES: Record<VenueCategory, readonly string[]> = {
  ...
  gym: ["GYM", "FITNESS", "TRAINING", "CLUB"],
  "boxing-gym": ["BOXING", "FIGHT CLUB", "RING", "GYM"],
  "shooting-range": ["RANGE", "ARMORY", "TRAINING", "LANES"],
  "weapon-shop": ["ARMS", "ARMORY", "WEAPONS", "SUPPLY"]
};
...
function categoriesForBuilding(building: BuildingState): VenueCategory[] {
  if (building.use === "retail") return ["convenience", "food", "clothing", "market", "pharmacy", "bar", "weapon-shop", "gym"];
  if (building.use === "mixed") return ["convenience", "food", "pharmacy", "clothing", "bar", "office-service", "gym", "boxing-gym"];
  ...
  if (building.use === "office" || building.use === "civic" || building.use === "education") return ["office-service", "food", "convenience", "gym", "shooting-range", "boxing-gym"];
  if (building.use === "hotel") return ["hotel", "food", "bar"];
  if (building.use === "entertainment") return ["entertainment", "bar", "food", "boxing-gym", "gym", "shooting-range"];
...
function venueTags(category: VenueCategory): string[] {
  const tags: Record<VenueCategory, string[]> = {
    ...
    gym: ["спортзал", "сила", "выносливость"],
    "boxing-gym": ["бокс", "спарринг", "ринг"],
    "shooting-range": ["тир", "стрельба", "оружие"],
    "weapon-shop": ["оружие", "броня", "экипировка"]
  };
...
function venueHours(category: VenueCategory, rng: SeededRandom): { openHour: number; closeHour: number } {
  ...
  if (category === "gym" || category === "boxing-gym") return { openHour: rng.integer(6, 9), closeHour: rng.integer(21, 24) };
  if (category === "shooting-range" || category === "weapon-shop") return { openHour: rng.integer(9, 12), closeHour: rng.integer(18, 22) };
...
function buildVenue(
  ...
  const category = anchorLocation ? categoryForLocation(anchorLocation) : categories.length ? categories[rng.integer(0, categories.length - 1)] : "office-service";
  ...
  return {
    ...
    mapPriority: anchorLocation ? 100 : category === "clinic" || category === "market" || category === "boxing-gym" ? 78 : category === "food" || category === "pharmacy" || category === "gym" || category === "shooting-range" || category === "weapon-shop" ? 68 : 48 + rng.integer(0, 18),
    operatingStatus,
...
/**
 * Builds the deterministic city-wide venue index without materializing interiors or
 * keeping every building in the streaming cache. The IDs are generated by the same
 * functions used by the active urban fabric, so a cold business resolves to the
 * exact venue, building and unit when its sector is later materialized.
 */
export function indexCitywideVenues(input: UrbanFabricInput, existing: VenueState[] = []): VenueState[] {
  const existingById = new Map(existing.map((venue) => [venue.id, venue]));
  const anchorBuildings = input.metropolitan.locations.flatMap((placement) => {
  ...
  const generatedBuildings: BuildingState[] = [];
  const anchorsBySector = new Map<string, BuildingState[]>();
  for (const building of anchorBuildings) {
    const group = anchorsBySector.get(building.sectorId);
    if (group) group.push(building);
    else anchorsBySector.set(building.sectorId, [building]);
  }

  for (const sector of input.metropolitan.sectors) {
    const catalog = catalogForSector(input.seed, input.timestamp, sector);
    const sectorAnchors = anchorsBySector.get(sector.id) ?? [];
    const sectorGenerated: BuildingState[] = [];
    const lotCapacity = lotsForSector(sector).length;
    const representedBusinessBuildings = sector.landUse === "commercial" || sector.landUse === "mixed"
      ? 2
      : sector.landUse === "industrial" || sector.landUse === "corporate" || sector.landUse === "civic" || sector.landUse === "transport"
        ? 1
        : sector.landUse === "residential"
          ? (new SeededRandom(`${input.seed}:city-business-index:${sector.id}`).chance(.34) ? 1 : 0)
          : 0;
    const target = Math.max(0, Math.min(catalog.buildingCount - sectorAnchors.length, lotCapacity, representedBusinessBuildings));
    let accepted = 0;
    for (let index = 0; index < Math.max(target, target * 3) && accepted < target; index += 1) {
      const candidate = generatedBuilding(input.seed, input.timestamp, sector, catalog, index, input.organizations, input.metropolitan.config.blockSizeM);
      const resolved = resolveBuildingPlacement(candidate, [...sectorAnchors, ...sectorGenerated], sector, input.seed);
      if (!resolved) continue;
      sectorGenerated.push({ ...resolved, detailLevel: "cold" });
      accepted += 1;
    }
    generatedBuildings.push(...sectorGenerated);
  }

  const venues: VenueState[] = [];
  for (const building of [...anchorBuildings, ...generatedBuildings]) {
    const anchorLocation = building.anchorLocationId ? input.locations.find((location) => location.id === building.anchorLocationId) : undefined;
    if (anchorLocation && anchorLocation.type !== "housing") {
      const generated = buildVenue(input, building, 0, anchorLocation);
      venues.push({ ...generated, ...existingById.get(generated.id), buildingId: building.id, unitId: generated.unitId, sectorId: building.sectorId, permanent: true, lastUpdatedAt: input.timestamp });
    }
    const count = venueCountForBuilding(building);
    for (let ordinal = 0; ordinal < count; ordinal += 1) {
      const generated = buildVenue(input, building, ordinal + (anchorLocation && anchorLocation.type !== "housing" ? 1 : 0));
      venues.push({ ...generated, ...existingById.get(generated.id), buildingId: building.id, unitId: generated.unitId, sectorId: building.sectorId, lastUpdatedAt: input.timestamp });
    }
  }

  const byId = new Map<string, VenueState>();
  for (const venue of venues) byId.set(venue.id, venue);
  for (const venue of existing) if (!byId.has(venue.id)) byId.set(venue.id, venue);
  return [...byId.values()].sort((left, right) => left.districtId.localeCompare(right.districtId) || left.sectorId.localeCompare(right.sectorId) || left.id.localeCompare(right.id));
}
...
function unitUseForVenue(category: VenueCategory): UnitUse {
  ...
  if (category === "repair" || category === "gym" || category === "boxing-gym" || category === "shooting-range") return "workshop";
...
export function createUrbanFabricState(input: UrbanFabricInput): UrbanFabricState {
  ...
  const venueOperations = createVenueOperationsState({
    ...
    externallyManaged: input.externallyManagedBusinessEconomy
  });
