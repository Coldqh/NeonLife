...
check("work screen contains only employment", work.includes("ВАКАНСИИ") && work.includes("Отработать смену") && !work.includes("TRAINING_ACTIONS") && !work.includes("EQUIPMENT_CATALOG") && !work.includes("boxing-fight") && !work.includes("street-fight"));
...
check("one-click work remains", system.includes('action.kind === "work-shift"') && system.includes("durationMinutes"));
...
check("new player-system styles are balanced", (css.match(/{/g) ?? []).length === (css.match(/}/g) ?? []).length);
...
console.log(`\n${passed}/${checks.length} world-bound player-system checks passed.`);
