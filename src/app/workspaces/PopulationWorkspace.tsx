...
export function PopulationWorkspace({ session }: { session: GameSession }) {
  ...
  const mobilitySnapshot = mobility.history[mobility.history.length - 1];
  const busiestMobilitySectors = mobility.sectorFlows.slice().sort((left, right) => right.congestionPercent - left.congestionPercent || right.roadDemandPerHour - left.roadDemandPerHour).slice(0, 14);
  const busiestRoutes = mobility.routes.slice().sort((left, right) => right.loadPercent - left.loadPercent || right.congestionPercent - left.congestionPercent).slice(0, 14);
  ...
  return (
    <div className="population-workspace">
      ...
      {tab === "mobility" ? (
        <section className="data-workspace metropolitan-workspace">
          <div className="population-labor__summary">
            <div><span>TRIPS / HOUR</span><strong>{mobilitySnapshot?.totalTripsPerHour.toLocaleString("ru-RU") ?? 0}</strong><small>{mobilitySnapshot?.roadTripsPerHour.toLocaleString("ru-RU") ?? 0} road · {mobilitySnapshot?.transitTripsPerHour.toLocaleString("ru-RU") ?? 0} transit</small></div>
            <div><span>CONGESTION</span><strong>{mobilitySnapshot?.averageCongestionPercent ?? 0}%</strong><small>peak {mobilitySnapshot?.peakCongestionPercent ?? 0}% · {mobilitySnapshot?.averageSpeedKph ?? 0} km/h</small></div>
            <div><span>TRANSIT LOAD</span><strong>{mobilitySnapshot?.averageTransitCrowdingPercent ?? 0}%</strong><small>{mobility.routes.filter((route) => route.primaryMode === "metro" || route.primaryMode === "bus").length} public routes</small></div>
            <div><span>PARKING</span><strong>{mobilitySnapshot?.parkingOccupancyPercent ?? 0}%</strong><small>{mobility.parking.reduce((sum, item) => sum + item.illegalParkingVehicles, 0).toLocaleString("ru-RU")} illegal vehicles</small></div>
            <div><span>COMMUTERS</span><strong>{mobility.commuterPlans.length}</strong><small>{mobility.commuterPlans.reduce((sum, item) => sum + item.tripsCompleted, 0).toLocaleString("ru-RU")} detailed trips completed</small></div>
            <div><span>FREIGHT</span><strong>{activeFreight.length} ACTIVE</strong><small>{mobility.totals.freightTrips.toLocaleString("ru-RU")} represented trips</small></div>
          </div>
          <div className="population-labor__columns">
            <section className="population-labor__list">
              <header><span>ROUTE NETWORK</span><strong>REAL SECTOR PATHS</strong></header>
              {busiestRoutes.map((route) => (
                <article key={route.id}>
                  <div><span>{route.code}</span><strong>{route.name}</strong><small>{route.kind.replace(/-/g, " ").toUpperCase()} · {(route.distanceM / 1000).toFixed(1)} km</small></div>
                  <div><span>LOAD</span><strong>{route.loadPercent}%</strong><small>{route.demandPerHour.toLocaleString("ru-RU")}/{route.capacityPerHour.toLocaleString("ru-RU")} per hour</small></div>
                  <div><span>TIME</span><strong>{route.currentDurationMinutes} MIN</strong><small>{route.primaryMode.toUpperCase()} · {route.status.toUpperCase()} · reliability {route.serviceReliability}%</small></div>
                </article>
              ))}
            </section>
            <section className="population-labor__list">
              <header><span>SECTOR TRAFFIC</span><strong>LIVE HOURLY FLOW</strong></header>
              {busiestMobilitySectors.map((flow) => {
                const sector = metropolitan.sectors.find((item) => item.id === flow.sectorId);
                return (
                  <article key={flow.sectorId}>
                    <div><span>{sector?.code ?? "SECTOR"}</span><strong>{districtName(session, flow.districtId)}</strong><small>{sector?.landUse.toUpperCase() ?? "UNKNOWN"} · {flow.roadDemandPerHour.toLocaleString("ru-RU")} road trips/h</small></div>
                    <div><span>CONGESTION</span><strong>{flow.congestionPercent}%</strong><small>{flow.averageSpeedKph} km/h</small></div>
                    <div><span>TRANSIT</span><strong>{flow.transitCrowdingPercent}%</strong><small>{flow.transitDemandPerHour.toLocaleString("ru-RU")} riders/h</small></div>
                  </article>
                );
              })}
            </section>
          ...
          <div className="population-labor__columns">
            <section className="population-labor__list">
              <header><span>FLEETS</span><strong>ACTIVE VEHICLES</strong></header>
              {majorFleets.map((fleet) => (
                <article key={fleet.id}>
                  <div><span>{fleet.mode.toUpperCase()}</span><strong>{fleet.activeVehicles.toLocaleString("ru-RU")} / {fleet.vehicles.toLocaleString("ru-RU")}</strong><small>{districtName(session, fleet.districtId)}</small></div>
                  <div><span>CAPACITY</span><strong>{fleet.capacityPerVehicle}</strong><small>per vehicle</small></div>
                  <div><span>RELIABILITY</span><strong>{fleet.serviceReliability}%</strong><small>condition {fleet.averageCondition}%</small></div>
                </article>
              ))}
            </section>
            <section className="population-labor__list">
              <header><span>PARKING PRESSURE</span><strong>PHYSICAL SPACES</strong></header>
              {strainedParking.map((parking) => {
                const sector = metropolitan.sectors.find((item) => item.id === parking.sectorId);
                return (
                  <article key={parking.id}>
                    <div><span>{sector?.code ?? "SECTOR"}</span><strong>{parking.occupiedSpaces.toLocaleString("ru-RU")} / {parking.spaces.toLocaleString("ru-RU")}</strong><small>{districtName(session, parking.districtId)} · {parking.status.toUpperCase()}</small></div>
                    <div><span>PRESSURE</span><strong>{parking.pressurePercent}%</strong><small>₵{parking.pricePerHour}/h</small></div>
                    <div><span>OVERFLOW</span><strong>{parking.illegalParkingVehicles}</strong><small>{parking.freightBays} freight bays</small></div>
                  </article>
                );
              })}
            </section>
          ...
          {activeFreight.length ? (
            <section className="population-labor__list">
              <header><span>FREIGHT IN MOTION</span><strong>PHYSICAL SHIPMENTS</strong></header>
              {activeFreight.map((movement) => (
                <article key={movement.id}>
                  <div><span>{movement.status.toUpperCase()}</span><strong>{movement.units.toLocaleString("ru-RU")} UNITS</strong><small>{movement.legality.toUpperCase()} · {movement.vehicleCount} vehicles</small></div>
                  <div><span>PATH</span><strong>{movement.pathSectorIds.length} SECTORS</strong><small>{movement.originSectorId.slice(-4).toUpperCase()} → {movement.destinationSectorId.slice(-4).toUpperCase()}</small></div>
                  <div><span>DELAY</span><strong>{movement.delayHours} H</strong><small>arrival {new Date(movement.estimatedArrivalAt).toLocaleString("ru-RU")}</small></div>
                </article>
              ))}
            </section>
          ) : null}
